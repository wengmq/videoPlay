...
void *CThreadPool::taskThread(void *arg)
{
	...
	//2.进入轮询工作模式，如果不退出且队列不为空一直工作
	while(1)
	{
		//.先去看下有没有任务，有任务则处理任务，没有在wait
		pool->m_cond.lock();//check task, if hasn't task, this thread turns to idle thread.
		if(pool->m_taskList.empty())
		{
			...
			if(!bSignal && pool->m_count > pool->m_mincount)
			{
				pool->m_count--;// Sub thread nums.
#ifdef THRADPOOL_DBG
				printf(" phread 0x%x is wait timedout.exit, count=%d\n", (int)pthread_self(), pool->m_count);
#endif
				pool->m_cond.unlock();
				pthread_exit(NULL);
			}
